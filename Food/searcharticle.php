...
if($by=="t")
{
$sql="SELECT title,id,url,author,DATE_FORMAT(date, '%D %M %Y'),aid,leadline,language FROM tbl_status WHERE type='f' AND status='a' AND language='e' AND title LIKE '%$key%';";
}
else if($by=="a")
{
$sql="SELECT title,id,url,author,DATE_FORMAT(date, '%D %M %Y'),aid,leadline,language FROM tbl_status WHERE type='f' AND status='a' AND language='e' AND author LIKE '%$key%';";
}
